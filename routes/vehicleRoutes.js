...
/* ============================================================
   🧾 ADD VEHICLE (Cloudinary Stream Upload)
============================================================ */
router.post(
  "/add",
  upload.fields([
...
/* ============================================================
   ❌ DELETE VEHICLE + CLOUDINARY IMAGES
============================================================ */
router.delete("/:id", async (req, res) => {
  try {
    const vehicle = await Vehicle.findById(req.params.id);
    ...
    if (vehicle.vehicleImages?.length > 0) {
      for (const url of vehicle.vehicleImages) {
    ...
    if (vehicle.rcFile) {
      ...
      await cloudinary.uploader.destroy(
        `atlms/vehicles/documents/${publicId}`,
        { resource_type: "raw" }
      );
    }
